...
class LCACalculation:
    ...
    def calculate_lcia_for_impact_data(impact_data: pd.DataFrame, impact_method_id: str,
        ...
        :param localize: set true to apply localizations
        ...
        for i, data_item in impact_data.iterrows():
            ...
            if not pd.isna(process_id):
                system_id = olca_ipc.create_product_system(process_id=process_id)
                if str(data_item[('Metadata', 'Localization')]).upper() == 'HU':
                    localize = True
                else:
                    localize = False
                ...
                olca_results[process_id] = res
        olca_result_all = pd.concat(olca_results, axis='columns').T

        # do the join on the openLCA id
        impact_data.index = impact_data[('Metadata', 'OlcaId')]
...
class OpenLCAIpc:
    ...
    def calculate_product_system(self, method: olca.ImpactMethod,
        ...
        if self.impact_db.has_table(table_name):
            ...
            if len(sys_result) > 0:
                if localize:
                    ...
                    sys_result = sys_result[sys_result['Localization'] == 'HU']
        ...
        if localize:
            ...
            if system.reference_process.location == 'HU':
                logger.debug('System is a local system')
            elif system.description is not None:
                if system.description.startswith('HU adapted'):
        ...
        if self.impact_db is not None:
            ...
            if system.reference_process.location == 'HU':
                result_to_db['Localization'] = 'HU'
            elif system.description is not None and system.description.startswith('HU adapted'):
                result_to_db['Localization'] = 'HU'
            else:
    ...
    def create_product_system(self, process_ref: olca.ProcessRef = None, process_name: str = None,
        ...
        if p_systems:  # non-empty dict
            for ps_name, ps_id in p_systems.items():
                if 'HU' in ps_name:
                    logger.info('Localized product system already exists with id: {id}'.format(id=ps_id))
                    return ps_id
            # if no localized system found return the first one
            for ps_name, ps_id in p_systems.items():
